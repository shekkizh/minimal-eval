...
from minieval.runner import _slug, run_attempt, run_suite
from minieval.sandbox import Sandbox, create_sandbox
from minieval.task import load_task
from minieval.types import CommandResult, Task
...
class HarnessTests(unittest.TestCase):
    ...
    def test_auto_vercel_drops_docker_options_and_reserves_grading_time(self):
        with patch.dict('os.environ', {'VERCEL_TOKEN': 'test'}), patch('minieval.sandbox.VercelSandbox') as backend:
            create_sandbox('auto', timeout_s=10, image='python:3.11-slim')
            backend.assert_called_once_with(timeout_ms=190000)
    ...
    def test_cleanup_failure_still_saves_artifacts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch('minieval.runner.create_sandbox', return_value=sandbox):
            result = run_attempt(Task('task', '', 'prompt', verifier='pass'), agent, 1, 'docker', 10, Path(tmp))
        ...
        for exit_code, verifier_exit in ((0, 0), (1, 0), (0, 1), (124, None)):
            with self.subTest(exit_code=exit_code, verifier_exit=verifier_exit), tempfile.TemporaryDirectory() as tmp:
                ...
                with patch('minieval.runner.create_sandbox', return_value=sandbox), \
                    ...
                    result = run_attempt(task, agent, 1, 'docker', 10, root / 'out')
    ...
    def test_existing_suite_is_not_overwritten(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(FileExistsError):
                run_suite([], [], 1, 'docker', 10, Path(tmp), run_name='saved')
